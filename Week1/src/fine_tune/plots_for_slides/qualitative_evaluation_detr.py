...
# Set paths
RESULTS_DIR = os.path.join(BASE_DIR, "results/detr/qualitative_evaluation")

# Models info
FINETUNED_WEIGHTS = os.path.join(BASE_DIR, "results/detr/detr_full_final/best_model.pth")
...
def main():
    ...
    model_pretrained = HuggingFaceDETR(weights=PRETRAINED_WEIGHTS, conf=0.5, device=device)
    model_finetuned = HuggingFaceDETR(weights=FINETUNED_WEIGHTS, conf=0.5, device=device)
    ...
    for i in indices:
        ...
        draw_boxes(axes[1], img, pred_pre["bboxes_xyxy"], pred_pre["category_ids"], scores=pred_pre["scores"], title="DETR Pre-Trained", is_mapped=True)
        
        # Fine-tuned (trained on mapped COCO labels, outputting COCO 1=Person, 3=Car natively)
        draw_boxes(axes[2], img, pred_ft["bboxes_xyxy"], pred_ft["category_ids"], scores=pred_ft["scores"], title="DETR Fine-Tuned (Full)", is_mapped=False)
