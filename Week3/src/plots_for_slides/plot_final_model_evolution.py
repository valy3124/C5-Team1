...
# Correct path using vali instead of benet
...
# Pick one good image and one bad image for evolution
target_images = ["VizWiz_val_00000102.jpg", "VizWiz_val_00000354.jpg"] 

output_dir = "/ghome/group01/C5/vali/C5-Team1/Week3/results/qualitative_final_model_evolution"
os.makedirs(output_dir, exist_ok=True)

for img_name in target_images:
    
    # Extract references from the last epoch (should be identical across epochs)
    references = []
    for s in data[str(epochs[-1])]:
        if s['image_name'] == img_name:
            references = s['references'][:4] # Take up to 4 to save space
            break
    ...
    y_pos -= 0.04
    axes[1].text(0.05, y_pos, "Evolution of Predictions:", fontsize=18, fontweight='bold', va='top')
    y_pos -= 0.05
...
print(f"Saved evolution plots to {output_dir}")
