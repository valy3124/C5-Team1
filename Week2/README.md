...
3. **Fine-Tuning SAM's Mask Decoder** on the KITTI-MOTS dataset (incorporating custom data augmentations) to improve boundary predictions for our specific classes while retaining SAM's zero-shot visual encoder features.
4. **Cross-Week Integration:** Using the bounding boxes generated by our best object detection model from Week 1 to prompt SAM, and comparing its performance against Grounded SAM's text-to-box approach.
5. **Generalization Analysis** by comparing zero-shot and fine-tuned results on the out-of-domain DEArt dataset.
6. **Semantic Segmentation (Optional Task):** Preprocessing instance masks into dense semantic maps and evaluating SAM using oracle box prompts and text prompts.
...
We fine-tune the SAM Mask Decoder using different types of prompts (Points, BBoxes, Text, or a uniform Mix during training). This pipeline includes data augmentations adapted for instance segmentation. Each strategy has an associated configuration file:
...
To evaluate the pretrained Baseline SAM using **point prompts** (Task a):
```bash
sbatch scripts/eval/run_eval_pretrained_points.sh
```
...
### Semantic Segmentation (Optional Task)

We extended our pipeline to perform Semantic Segmentation. This includes preprocessing instance masks into dense semantic maps, passing class labels to the model, and resolving overlaps by ranking SAM's internal confidence scores.
```bash
sbatch scripts/inference/run_semantic_segmentation.sh
```
